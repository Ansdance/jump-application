...
class PlayerResourceIT {
    ...
    void createPlayer() throws Exception {
        ...
        var returnedPlayerDTO = om.readValue(
            restPlayerMockMvc
                .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(playerDTO)))
    ...
    void putExistingPlayer() throws Exception {
        ...
        updatedPlayer.firstName(UPDATED_FIRST_NAME).lastName(UPDATED_LAST_NAME).email(UPDATED_EMAIL).phoneNumber(UPDATED_PHONE_NUMBER);
    ...
    void putWithMissingIdPathParamPlayer() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPlayerMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(playerDTO)))
    ...
    void partialUpdatePlayerWithPatch() throws Exception {
        ...
        partialUpdatedPlayer.phoneNumber(UPDATED_PHONE_NUMBER);
    ...
    void patchWithMissingIdPathParamPlayer() throws Exception {
        ...
        restPlayerMockMvc
            .perform(
                patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(playerDTO))
            )
            .andExpect(status().isMethodNotAllowed());
